...
class NdExpr(TensorExpr, abc.ABC):
    ...
    def nd_free_indices(self) -> Iterable[Index]:
        """Return all the indices used in this Nd_expression
         that have not been explicitly contracted or used to produce the inner nd_space
        """
        pass
...
class IndexedNDTensor(NdExpr):
    ...
    def __str__(self) -> str:
        ...
        return f"{self.scalar_expr}/{','.join(str(idx) for idx in self.nd_space_indices)}/"
...
class NDScalarExpr(ScalarExpr):
    ...
    def free_indices(self) -> Iterable[Index]:
        return frozenset(self.nd_expr.nd_free_indices) | frozenset(self.nd_expr.nd_space_indices)
