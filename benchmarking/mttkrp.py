...
_Args = tuple[Any, StructType, StructType, StructType, StructType]

class Mttkrp(Benchmark, abc.ABC):
    def __init__(
        self,
        i: int,
        j: int,
        ...
        **kwargs,
        ...
        new_base_dir = f"{base_dir}/mttkrp"
        results_base_dir = (
            f"{new_base_dir}/{name}_{var_name}_O{opt_num}_{i}.{j}.{k}.{l}_{seed}_{runs}"
        )
        ...
        np_a, np_b, np_c = self.make_a_b_c(r)

        # print("generating np c")
        np_d = np.einsum("ikl,lj,kj->ij", np_a, np_b, np_c)
        ...
        self.handle_reference_array(np_c, "np_c" + np_suffix, True, False, "np_c")
        self.handle_reference_array(np_d, "np_d" + np_suffix, False, True, "np_d")

    def make_a_b_c(self, r: Random) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
        np_a = np.zeros((self.i, self.k, self.l), dtype=np.float32)
        np_b = np.zeros((self.l, self.j), dtype=np.float32)
        np_c = np.zeros((self.k, self.j), dtype=np.float32)
        for i_i in range(self.i):
            for i_k in range(self.k):
                for i_l in range(self.l):
                    num = r.random()
                    np_a[i_i, i_k, i_l] = num
        for i_l in range(self.l):
            for i_j in range(self.j):
                num = r.random()
                np_b[i_l, i_j] = num
    ...
    def get_configs_for_tensors(
        self, a: TensorVariable, b: TensorVariable, c: TensorVariable, d: TensorVariable
    ...
    def define_lib_builder(self) -> LibBuilder:
        if self.use_scope_vars:
            ...
            vl = UnknownSizeVectorSpace("vl")
            scope_var_map = {vi: self.i, vj: self.j, vk: self.k, vl: self.l}
        else:
            vi = RealVectorSpace(self.i)
            vj = RealVectorSpace(self.j)
            vk = RealVectorSpace(self.k)
            vl = RealVectorSpace(self.l)
            scope_var_map = {}
        A = TensorVariable(vi * vk * vl, "A")
        B = TensorVariable(vl * vj, "B")
        C = TensorVariable(vk * vj, "C")
        D = TensorVariable(vi * vj, "D")

        self.tensor_variables = (A, B, C, D)
        ...
        mttkrp = (A[_i,_k,_l] * B[_l,_j] * C[_k,_j]).sum(_k,_l).forall(_i, _j)

        lib_builder = LibBuilder(scope_var_map)
        self.construct_lib_builder(lib_builder, A, B, C, D)

        lib_builder.make_setter("set_A", (A), {}, [0, 1, 2])
        lib_builder.make_setter("set_B", (B), {}, [0, 1])
        lib_builder.make_setter("set_C", (C), {}, [0, 1])

        lib_builder.make_getter("get_D", (D), {}, [0, 1])
        lib_builder.make_function("mttkrp", mttkrp, [D], [A, B, C], [], [])
        lib_builder.make_function("copy_A", A, [A], [A], [], [])
        lib_builder.make_function("copy_B", B, [B], [B], [], [])
        lib_builder.make_function("copy_C", C, [C], [C], [], [])
